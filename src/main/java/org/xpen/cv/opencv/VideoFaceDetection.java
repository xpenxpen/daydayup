...
import static org.bytedeco.javacpp.opencv_core.cvGetTickCount;
import static org.bytedeco.javacpp.opencv_core.cvGetTickFrequency;
...
import static org.bytedeco.javacpp.opencv_videoio.cvCaptureFromAVI;
import static org.bytedeco.javacpp.opencv_videoio.cvCreateAVIWriter;
...
import org.bytedeco.javacpp.opencv_videoio.CvVideoWriter;
...
public class VideoFaceDetection {
    ...
    public static void main(String[] args) {
        ...
        if (args.length >= 1) {
            ...
            vidfps = cvGetCaptureProperty(camera, CV_CAP_PROP_FPS);
        ...
        while (flag) {
            ...
            detectFaceInImage(frame, frame_bw, cascade, model, bbox, landmarks);
    ...
    private static void detectFaceInImage(IplImage orig, IplImage input, CvHaarClassifierCascade cascade, FLANDMARK_Model model, int[] bbox, double[] landmarks) {
        // Smallest face size.
        ...
        CvSeq rects = cvHaarDetectObjects(input, cascade, storage, 1.1, 3, CV_HAAR_DO_CANNY_PRUNING);
        cvClearMemStorage(storage);
        
        int nFaces = rects.total();
    
        for (int iface = 0; iface < nFaces; iface++) {
            CvRect r = new CvRect(cvGetSeqElem(rects, iface));
